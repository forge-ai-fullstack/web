...
COUNTER_FILE = "htpc/iptv/tmp/fofa/计数.txt"
IP_DIR = "htpc/iptv/tmp/fofa/ip"
RTP_DIR = "htpc/iptv/tmp/fofa/rtp"
ZUBO_FILE = "htpc/iptv/tmp/fofa/zubo.txt"
IPTV_FILE = "htpc/iptv/tmp/fofa/fofa.txt"
...
CHANNEL_CATEGORIES = {
    "央视": [
        ...
        "中国教育1台", "中国教育2台", "中国教育3台", "中国教育4台", "早期教育", "CHC动作电影", "CHC家庭影院", "CHC影迷电影", 
        "淘电影", "淘精彩", "淘剧场", "淘4K", "淘娱乐", "淘BABY", "淘萌宠", "重温经典", "求索纪录", "求索科学",
        "求索生活", "求索动物", "快乐垂钓", "茶频道", "先锋乒羽", "天元围棋", "汽摩", "梨园频道", "文物宝库", "武术世界", "哒啵赛事", "哒啵电竞", "黑莓电影", "黑莓动画", 
        "乐游", "生活时尚", "都市剧场", "欢笑剧场", "游戏风云", "金色学堂", 
    ...
    "卫视": [
        ...
        "动漫秀场", "新动漫", "卡酷少儿", "金鹰卡通", "优漫卡通", "哈哈炫动", "嘉佳卡通"
    ],
    "asia": [
        "星空卫视", "CHANNEL[V]", "凤凰中文", "凤凰资讯", "凤凰香港", "凤凰电影"
    ],
    "体育": [
         "睛彩青少", "睛彩竞技", "睛彩篮球", "睛彩广场舞", "魅力足球", "五星体育",
        "劲爆体育" 
    ],#任意添加，与仓库中rtp/省份运营商.txt内频道一致即可，或在下方频道名映射中改名
    "体育": [
        "纪实人文", "金鹰纪实", "纪实科教","山东教育卫视"
    ]
}
...
# 第三阶段
def third_stage():
    print("🧩 第三阶段：多线程检测代表频道生成 fofa.txt")
    ...
    print(f"✅ 检测完成，可播放IP共 {len(playable_ips)} 个")
    ...
    print(f"🎯 fofa.txt 生成完成（含更新时间），共 {len(valid_lines)} 条频道")
...
def push_all_files():
    ...
    os.system("git add htpc/iptv/tmp/fofa/计数.txt")
    os.system("git add htpc/iptv/tmp/fofa/ip/*.txt || true")
    os.system("git add htpc/iptv/tmp/fofa/fofa.txt || true")
    os.system('git commit -m "自动更新：计数、IP文件、fofa.txt" || echo "⚠️ 无需提交"')
